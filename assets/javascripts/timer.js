(function($, undefined) {
	...
	$.timerlog_timer_i18n = {
		...
		_ : function(key) {
			key += '';
			var type = (typeof this._p[key]), _args = Array.prototype.slice
					.call(arguments);
			_args.shift();
			switch (type) {
			case 'undefined':
		...
		_format : function() {
			var _args = Array.prototype.slice.call(arguments), _p = _args
					.shift(), len = _args.length;

			while (len--) {
				_p = _p.replace(new RegExp("\\{" + len + "\\}", "g"),
						_args[len]);
			}
		...
		/**
		 * Format language plural form Params 3-5 can omitted 3 params calls,
		 * second param used for 1 and third params for others 4 params calls,
		 * second param used for 1, third params for 2-4, fourth param for 0, 5+
		 * 5 params calls, second param used for 0, third params for
		 * [1,21,31...], fourth param for 2-4, fifth param for 5+
		 *
		 * @param {int|float}
		 *            num
		 * @param {String}
		...
		plural : function(num, zero, one, few, many, other) {
			...
			var _num = parseInt(num), _float = parseFloat(num) - _num, _args = Array.prototype.slice
					.call(arguments).slice(1), str;
	...
	function timerlog_timer() {
		var val, timer, startTime, updateInterval = 30 * 1000, fieldSelector = '#time_entry_hours', field = $(
				fieldSelector).eq(0), event_namespace = '.timerlog_timer', storageName = 'timerlog_timer', lang = $.timerlog_timer_i18n, funcs = {
			init : function() {
			},
			startTimer : function() {
			},
		...
		}, button, text, storageChecked, form, initialVal, initiated;

		if (field.length) {
			field.attr('autocomplete', 'off');
			funcs.init = function() {
				var tpl = $('<span class="timerlog-timer"> <button class="timerlog-timer-button"></button>'
						+ ' <em class="timerlog-timer-text"><small><span></span></small></em></span>');
			...
			funcs.stopTimer = function(notSaveStorage) {
				...
				if (!notSaveStorage) {
					saveTimer(undefined, getVal(getVal(field.val())
							- initialVal));
				}
			};
			...
			funcs.tick = function() {
				if (startTime) {
					var diff = getTimestamp() - startTime, secs = diff / 3600, new_val = getVal(val
							+ secs);
			...
			funcs.init = function() {
				...
				if (data) {
					var tpl = $('<span style="margin-left:1em">'
							+ lang._('title')
							+ ': <a class="timerlog-timer-text"></a></span>'), a = tpl
							.find('a');
					...
					if (data.t) {
						var tick = function() {
							var diff = getTimestamp() - data.t, hours = diff / 3600, new_val = getVal(data.h
									+ hours);
		...
		if (field.length) {
			...
			if (up_block.length && !up_block.is(':visible')) {
				$('.icon.icon-edit').bind('click' + event_namespace,
						function() {
							init();
		...
		function format_time(val) {
			var h = parseInt(val, 10), m = parseInt((val - h) * 60, 10);
			return (h ? lang._('hours', h) + ' ' : '')
					+ lang._('minutes', (m > 0 && m < 10) ? '0' + m : m);
		}
